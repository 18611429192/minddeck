  ...
  function aiElementSnapshotV10(element){
    ...
    if(element.type==='image')return {id:element.id,type:'image',image:{src:String(element.src||'').slice(0,12000),alt:String(element.alt||'').slice(0,1000),fit:element.fit||'cover',objectPosition:element.objectPosition||'50% 50%'}};
    return null;
  }
  ...
  function aiSyncStructuredContentV10(node,element){
    ...
    if(element.type==='chart'){const value=Core.NativeChart.normalize(element);node.composer.content.chart={chartType:value.chartType,categories:value.categories.slice(),labels:value.labels.slice(),values:value.values.slice(),series:value.series.map(item=>({id:item.id,name:item.name,values:item.values.slice()})),options:{...value.options}}}
      ...
      const media=Array.isArray(node.composer.content.media)?node.composer.content.media:[];
      const current=media.find(item=>item.id===element.id)||media.find(item=>item.src===element.src)||media.find(item=>item.type==='image');
      const next={...(current||{}),type:'image',src:element.src,alt:element.alt||'',fit:element.fit||'cover',objectPosition:element.objectPosition||'50% 50%'};
      node.composer.content.media=current?media.map(item=>item===current?next:item):[...media,next];
    }
  }
  function aiApplyElementPatchV10(node,item){
    ...
    else if(item.type==='image'&&item.image){element.src=item.image.src;element.alt=item.image.alt??element.alt;element.fit=item.image.fit||element.fit||'cover';element.objectPosition=item.image.objectPosition||element.objectPosition||'50% 50%';aiSyncStructuredContentV10(node,element)}
  }
