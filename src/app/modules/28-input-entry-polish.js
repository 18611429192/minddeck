  ...
  function inputEntryPolish(){
    ...
    smart.addEventListener('click',()=>setTimeout(()=>{
      ...
      const patch=()=>{const block=[...preview.children].find(element=>element.tagName==='DIV'&&/\d+ 个结构模板/.test(element.textContent||''));if(block)block.textContent=(block.textContent||'').replace(/\d+ 个结构模板/,`${ComposerV99.templates.length} 个结构模板`)};
