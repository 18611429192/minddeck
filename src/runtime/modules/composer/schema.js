import { PAGE_ROLES, ComposerRoleSet, composerClean, composerHashString, composerNumericPart } from './base.js';
...
export function normalizeSlideContent(input={}){
  ...
  return {
    ...
    items:items.map((item,index)=>{const src=item&&typeof item==='object'&&!Array.isArray(item)?item:{label:item};const label=composerClean(src.label??src.title??src.text),value=src.value===undefined||src.value===null?'':composerClean(src.value),detail=composerClean(src.detail),unit=composerClean(src.unit),image=composerClean(src.image);return {id:composerClean(src.id)||`item-${index+1}-${composerHashString(label||value||index)}`,label,value,detail,unit,image}}).filter(item=>item.label||item.value||item.detail||item.image),
    media:media.map((item,index)=>{const src=item&&typeof item==='object'&&!Array.isArray(item)?item:{},type=src.type==='video'?'video':'image';return {id:composerClean(src.id)||`media-${index+1}`,type,src:composerClean(src.src),alt:composerClean(src.alt)}}).filter(item=>item.src)
  };
}
...
function normalizePlanningMetadata(value){if(!value||typeof value!=='object'||Array.isArray(value))return value;return {...(value.requestedTargetSlides!==undefined?{requestedTargetSlides:value.requestedTargetSlides}:{}),...(value.actualSlides!==undefined?{actualSlides:value.actualSlides}:{}),...(value.warnings!==undefined?{warnings:Array.isArray(value.warnings)?value.warnings.map(item=>item&&typeof item==='object'&&!Array.isArray(item)?{...item}:item):value.warnings}:{})}}
export function validateDeckSpec(input){const errors=[],warnings=[],push=(path,code,message)=>errors.push({path,code,message});if(!input||typeof input!=='object'||Array.isArray(input)){push('$','SPEC_TYPE','DeckSpec must be an object');return {ok:false,errors,warnings}}if(input.schemaVersion!==undefined&&input.schemaVersion!==1)push('schemaVersion','SCHEMA_VERSION','Only DeckSpec v1 is supported');if(!composerClean(input.title))push('title','TITLE_REQUIRED','title is required');if(!composerClean(input.goal))push('goal','GOAL_REQUIRED','goal is required');if(input.coverOnly!==undefined&&typeof input.coverOnly!=='boolean')push('coverOnly','COVER_ONLY_TYPE','coverOnly must be boolean');if(input.planning!==undefined){if(!input.planning||typeof input.planning!=='object'||Array.isArray(input.planning))push('planning','PLANNING_TYPE','planning must be an object');else{if(!Number.isInteger(input.planning.requestedTargetSlides)||input.planning.requestedTargetSlides<1||input.planning.requestedTargetSlides>60)push('planning.requestedTargetSlides','PLANNING_REQUESTED_SLIDES','planning.requestedTargetSlides must be 1..60');if(!Number.isInteger(input.planning.actualSlides)||input.planning.actualSlides<1||input.planning.actualSlides>60)push('planning.actualSlides','PLANNING_ACTUAL_SLIDES','planning.actualSlides must be 1..60');if(!Array.isArray(input.planning.warnings))push('planning.warnings','PLANNING_WARNINGS','planning.warnings must be an array')}}if(!Array.isArray(input.slides)||(!input.slides.length&&input.coverOnly!==true))push('slides','SLIDES_REQUIRED','slides must contain at least one slide unless coverOnly is true');const ids=new Set();for(const [index,slide] of (Array.isArray(input.slides)?input.slides:[]).entries()){const path=`slides[${index}]`;if(!slide||typeof slide!=='object'||Array.isArray(slide)){push(path,'SLIDE_TYPE','slide must be an object');continue}const id=composerClean(slide.id);if(id){if(ids.has(id))push(`${path}.id`,'DUPLICATE_SLIDE_ID',`duplicate slide id: ${id}`);ids.add(id)}if(slide.role!==undefined&&!ComposerRoleSet.has(slide.role))push(`${path}.role`,'INVALID_ROLE',`unknown role: ${slide.role}`);if(!slide.content||typeof slide.content!=='object'||Array.isArray(slide.content))push(`${path}.content`,'CONTENT_REQUIRED','content object is required')}return {ok:errors.length===0,errors,warnings}}
export function normalizeDeckSpec(input={},options={}){const source=input&&typeof input==='object'&&!Array.isArray(input)?input:{},slides=Array.isArray(source.slides)?source.slides:[];return {schemaVersion:1,title:composerClean(source.title)||composerClean(options.title)||'未命名演示',goal:composerClean(source.goal)||composerClean(options.goal)||'清晰表达核心内容',audience:composerClean(source.audience),theme:composerClean(source.theme)||composerClean(options.theme)||'aurora',randomSeed:composerClean(source.randomSeed)||composerClean(options.seed)||composerHashString(source.title||'minddeck'),...(source.coverOnly===true?{coverOnly:true}:{}),...(source.planning!==undefined?{planning:normalizePlanningMetadata(source.planning)}:{}),slides:slides.map((slide,index)=>{const src=slide&&typeof slide==='object'&&!Array.isArray(slide)?slide:{},content=normalizeSlideContent({...src.content,title:src.content?.title||src.title}),id=composerClean(src.id)||`slide-${index+1}-${composerHashString(content.title||index)}`;return {id,title:content.title,role:ComposerRoleSet.has(src.role)?src.role:inferSlideRole(content,null,{depth:1,index}),content}})}}
