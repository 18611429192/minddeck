...
export function validateDeckSpec(input){const errors=[],warnings=[],push=(path,code,message)=>errors.push({path,code,message});if(!input||typeof input!=='object'||Array.isArray(input)){push('$','SPEC_TYPE','DeckSpec must be an object');return {ok:false,errors,warnings}}if(input.schemaVersion!==undefined&&input.schemaVersion!==1)push('schemaVersion','SCHEMA_VERSION','Only DeckSpec v1 is supported');if(!composerClean(input.title))push('title','TITLE_REQUIRED','title is required');if(!composerClean(input.goal))push('goal','GOAL_REQUIRED','goal is required');if(!Array.isArray(input.slides))push('slides','SLIDES_REQUIRED','slides must be an array');else if(!input.slides.length&&input.coverOnly!==true)push('slides','SLIDES_REQUIRED','slides must contain at least one slide unless coverOnly is true');if(input.coverOnly!==undefined&&typeof input.coverOnly!=='boolean')push('coverOnly','COVER_ONLY_TYPE','coverOnly must be boolean');const ids=new Set();for(const [index,slide] of (Array.isArray(input.slides)?input.slides:[]).entries()){const path=`slides[${index}]`;if(!slide||typeof slide!=='object'||Array.isArray(slide)){push(path,'SLIDE_TYPE','slide must be an object');continue}const id=composerClean(slide.id);if(id){if(ids.has(id))push(`${path}.id`,'DUPLICATE_SLIDE_ID',`duplicate slide id: ${id}`);ids.add(id)}if(slide.role!==undefined&&!ComposerRoleSet.has(slide.role))push(`${path}.role`,'INVALID_ROLE',`unknown role: ${slide.role}`);if(!slide.content||typeof slide.content!=='object'||Array.isArray(slide.content))push(`${path}.content`,'CONTENT_REQUIRED','content object is required')}return {ok:errors.length===0,errors,warnings}}
export function normalizeDeckSpec(input={},options={}){const source=input&&typeof input==='object'&&!Array.isArray(input)?input:{},slides=Array.isArray(source.slides)?source.slides:[],normalized={schemaVersion:1,title:composerClean(source.title)||composerClean(options.title)||'未命名演示',goal:composerClean(source.goal)||composerClean(options.goal)||'清晰表达核心内容',audience:composerClean(source.audience),theme:composerClean(source.theme)||composerClean(options.theme)||'aurora',randomSeed:composerClean(source.randomSeed)||composerClean(options.seed)||composerHashString(source.title||'minddeck'),slides:slides.map((slide,index)=>{const src=slide&&typeof slide==='object'&&!Array.isArray(slide)?slide:{},content=normalizeSlideContent({...src.content,title:src.content?.title||src.title}),id=composerClean(src.id)||`slide-${index+1}-${composerHashString(content.title||index)}`;return {id,title:content.title,role:ComposerRoleSet.has(src.role)?src.role:inferSlideRole(content,null,{depth:1,index}),content}})};if(source.coverOnly===true)normalized.coverOnly=true;return normalized}
