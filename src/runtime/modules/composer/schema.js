...
function pushType(errors,path,code,message){errors.push({path,code,message})}
function validateSlideContent(content,path,errors){
  if(!isObject(content)){pushType(errors,path,'CONTENT_TYPE','content must be an object');return}
  for(const key of ['title','subtitle','summary','takeaway']){
    if(content[key]!==undefined&&!isString(content[key]))pushType(errors,`${path}.${key}`,'CONTENT_FIELD_TYPE',`${key} must be a string`);
  }
  if(content.items!==undefined&&!Array.isArray(content.items))pushType(errors,`${path}.items`,'ITEMS_TYPE','items must be an array');
  for(const [index,item] of (Array.isArray(content.items)?content.items:[]).entries()){
    const itemPath=`${path}.items[${index}]`;
    if(!isObject(item)){pushType(errors,itemPath,'ITEM_TYPE','item must be an object');continue}
    for(const key of ['id','label','detail','unit','image']){
      if(item[key]!==undefined&&!isString(item[key]))pushType(errors,`${itemPath}.${key}`,'ITEM_FIELD_TYPE',`${key} must be a string`);
    }
    if(item.value!==undefined&&!isString(item.value)&&typeof item.value!=='number')pushType(errors,`${itemPath}.value`,'ITEM_FIELD_TYPE','value must be a string or number');
  }
  if(content.media!==undefined&&!Array.isArray(content.media))pushType(errors,`${path}.media`,'MEDIA_TYPE','media must be an array');
  for(const [index,media] of (Array.isArray(content.media)?content.media:[]).entries()){
    const mediaPath=`${path}.media[${index}]`;
    if(!isObject(media)){pushType(errors,mediaPath,'MEDIA_ITEM_TYPE','media item must be an object');continue}
    if(media.id!==undefined&&!isString(media.id))pushType(errors,`${mediaPath}.id`,'MEDIA_FIELD_TYPE','id must be a string');
    if(media.type!==undefined&&!['image','video'].includes(media.type))pushType(errors,`${mediaPath}.type`,'MEDIA_KIND','media.type must be image or video');
    if(media.src!==undefined&&!isString(media.src))pushType(errors,`${mediaPath}.src`,'MEDIA_FIELD_TYPE','media.src must be a string');
    if(media.alt!==undefined&&!isString(media.alt))pushType(errors,`${mediaPath}.alt`,'MEDIA_FIELD_TYPE','media.alt must be a string');
  }
}
export function normalizeSlideContent(input={}){
  ...
  return {
    ...
    items:items.map((item,index)=>{const src=isObject(item)?item:{label:item};const label=composerClean(src.label??src.title??src.text),value=src.value===undefined||src.value===null?'':composerClean(src.value),detail=composerClean(src.detail),unit=composerClean(src.unit),image=composerClean(src.image);return {id:composerClean(src.id)||`item-${index+1}-${composerHashString(label||value||index)}`,label,value,detail,unit,image}}).filter(item=>item.label||item.value||item.detail||item.image),
    media:media.map((item,index)=>{const src=isObject(item)?item:{},type=src.type==='video'?'video':'image';return {id:composerClean(src.id)||`media-${index+1}`,type,src:composerClean(src.src),alt:composerClean(src.alt)}}).filter(item=>item.src)
  };
}
...
export function validateDeckSpec(input){
  ...
  if(!isObject(input)){push('$','SPEC_TYPE','DeckSpec must be an object');return {ok:false,errors,warnings}}
  if(input.schemaVersion!==1)push('schemaVersion','SCHEMA_VERSION','schemaVersion must be 1');
  if(!isString(input.title))push('title','TITLE_TYPE','title must be a string');else if(!input.title.trim())push('title','TITLE_REQUIRED','title is required');
  if(!isString(input.goal))push('goal','GOAL_TYPE','goal must be a string');else if(!input.goal.trim())push('goal','GOAL_REQUIRED','goal is required');
  if(input.audience!==undefined&&!isString(input.audience))push('audience','AUDIENCE_TYPE','audience must be a string');
  if(input.theme!==undefined){if(!isString(input.theme))push('theme','THEME_TYPE','theme must be a string');else if(!ComposerThemeMap[input.theme])push('theme','INVALID_THEME',`unknown theme: ${input.theme}`)}
  if(input.randomSeed!==undefined&&!isString(input.randomSeed))push('randomSeed','RANDOM_SEED_TYPE','randomSeed must be a string');
  ...
  for(const [index,slide] of input.slides.entries()){
    ...
    if(!isObject(slide)){push(path,'SLIDE_TYPE','slide must be an object');continue}
    if(!isString(slide.id))push(`${path}.id`,'SLIDE_ID_TYPE','slide id must be a string');
