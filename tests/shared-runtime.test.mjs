...
assert.equal(Core.VERSION,'9.6');
for(const name of ['Ids','Tree','Project','Layout','Commands','Presentation','PresentationSession','Stage','MapViewport','Animation','Element','Slide','Fullscreen','Input','Recovery','Diagnostics','InlineEditor','MapRenderer','TocRenderer','ExportData','Portable','Architecture']) assert.ok(Core[name],`missing core ${name}`);
...
assert.deepEqual(Object.keys(Core.Architecture.singleSources).sort(),['animation','commands','ids','diagnostics','fullscreen','input','layout','mapRender','portable','presentation','project','recovery','slideRender','theme','tocRender','tree','viewport'].sort());
