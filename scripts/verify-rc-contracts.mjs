...
if (!versionMatch) throw new Error(`package.json version is not an RC version: ${pkg.version}`);

const [, major, minor, patch] = versionMatch;
const displayVersion = `MindDeck V${major}.${minor}.${patch} RC`;
if (!html.includes(displayVersion)) {
  throw new Error(`RC version mismatch: package.json=${pkg.version}; expected index.html to contain "${displayVersion}"`);
}
...
for (const marker of ['TODO_RELEASE_BLOCKER','FIXME_RELEASE_BLOCKER']) {
